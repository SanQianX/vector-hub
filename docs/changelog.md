...
New `MiniMaxEmbeddings` class (exported from both `vectra` and `vectra/browser`) for MiniMax hosted embedding models. The default model is `embo-01` (1536 dimensions) and the default endpoint is `https://api.minimax.cn`.

```ts
import { LocalDocumentIndex, MiniMaxEmbeddings } from 'vectra';

const docs = new LocalDocumentIndex({
  folderPath: './my-index',
  embeddings: new MiniMaxEmbeddings({ apiKey: process.env.MINIMAX_API_KEY! }),
});
```

**Who is affected:** Callers that want MiniMax embeddings, including Coding/Token Plan subscription keys (`sk-cp-...`) — plan usage deducts from the subscription's shared quota. No existing code changes; the class is additive.

Notes:
- The MiniMax embeddings API is not OpenAI-compatible (it expects `texts` + `type: 'db'|'query'` and returns a top-level `vectors` array), so it cannot be used through `OpenAIEmbeddings`; this adapter handles the format mapping.
- The adapter maps string input to `type: 'query'` and array input to `type: 'db'`, matching how `LocalDocumentIndex` calls the model (single query string for `queryDocuments()`, array of chunks for `upsertDocument()`).
- Errors are often returned with HTTP 200 and a non-zero `base_resp.status_code`; these are surfaced as `status: 'error'` with the code and message.
